...
func statusCheck(db *sqlx.DB) error {
	...
	for attempts < 5 {
		if err := db.Ping(); err != nil {
			fmt.Printf("failed ping database, err:%v, attemps: %d\n", err, attempts)
		...
		const q = `SELECT TRUE`
		var tmp bool
		if err := db.QueryRow(q).Scan(&tmp); err != nil {
			fmt.Printf("failed to do test scan database, err: %v\n", err)
			time.Sleep(time.Duration(attempts) * time.Second)
			attempts++
			continue
		}
	...
	return errors.New("failed to check database connect, all attempts exceeded")
}
...
func NamedExecContext(ctx context.Context, log *logger.Logger, db sqlx.ExtContext, query string, data any) (err error) {
	...
	ctx, span := trace.NewSpan(ctx, "database.NamedExecContext")
	defer span.End()
...
func namedQuerySlice[T any](ctx context.Context, log *logger.Logger, db sqlx.ExtContext, query string, data any, dest *[]T, withIn bool) (err error) {
	...
	ctx, span := trace.NewSpan(ctx, "database.namedQuerySlice")
	defer span.End()
